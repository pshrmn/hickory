import "jest";
import { blockingInMemory } from "../src";
...
describe("Memory constructor", () => {
  it("initializes with root location (/) if none provided", () => {
    const testHistory = blockingInMemory(pending => {
      pending.finish();
    });
  ...
  it("works with string locations", () => {
    const testHistory = blockingInMemory(
      pending => {
        pending.finish();
      },
      {
...
describe("go", () => {
  ...
  describe("with a value", () => {
    it("does nothing if the value is outside of the range", () => {
      ...
      const testHistory = blockingInMemory(router);
      testHistory.go(10);
